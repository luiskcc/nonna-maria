class ClientFeedbackMailer < ApplicationMailer
    ...
    recipients = [ENV['ADMIN_EMAIL'], ENV['ADMIN_EMAIL_2'], ENV['ADMIN_EMAIL_3'], ENV['ADMIN_EMAIL_4']].compact
    ...
    def new_feedback_email(feedback_data)
        ...
        if @rating.present? && @client_name.present?
            mail(to: recipients, subject: "⚠️ Nuevo Comentarion de  *##{@rating} ESTRELLAS de: #{@client_name}")
        else
